...
int caf_plotter(std::string input_file_list, std::string output_rootfile, bool dataOnly) {
	...
	// Add cathode-specific regions
	const double cathode_x_min_neg = -40; // Cathode region near -35
	const double cathode_x_max_neg = -30;
	const double cathode_x_min_pos = 30;  // Cathode region near +35
	const double cathode_x_max_pos = 40;
    ...
    for (long n = 0; n < Nentries; n++) {
        ...
        for (long unsigned nixn = 0; nixn < sr->common.ixn.dlp.size(); nixn++) {
			...
			// Conditions for All Modules
			// if (abs(sr->common.ixn.dlp[nixn].vtx.x) > 59 || 
				...
				// continue; // Skip if outside fiducial volume
			// }

			// Now apply the negative cathode-specific region condition
			// if (!(reco_nu_vtxX >= cathode_x_min_neg && reco_nu_vtxX <= cathode_x_max_neg)) {
				// continue; // Skip if not in negative cathode region
			// }
			
			// Now apply the positive cathode-specific region condition
			// if (!(reco_nu_vtxX >= cathode_x_min_pos && reco_nu_vtxX <= cathode_x_max_pos)) {
				// continue; // Skip if not in negative cathode region
			// }

			// Now apply the both cathode-specific region condition
			// if (!((reco_nu_vtxX >= cathode_x_min_neg && reco_nu_vtxX <= cathode_x_max_neg) || (reco_nu_vtxX >= cathode_x_min_pos && reco_nu_vtxX <= cathode_x_max_pos) )) {
				// continue; // Skip if not in negative cathode region
			// }
		

			// If the code reaches this point, it means the entry passes both conditions
...
void Save2DHistogramAsPNG(TH2* histogram, const std::string& suffix) {
    ...
    gStyle->SetOptStat(1); // Turn off statistics box
